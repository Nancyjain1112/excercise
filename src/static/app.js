    ...
    try {
            ...
            <div class="participants-section">
              ...
              <ul class="participants-list">
                ${participants
                  .map(
                    (participant) => `
                      <li class="participant-pill" data-activity="${escapeHtml(name)}" data-email="${escapeHtml(participant)}">
                        <span class="participant-name">${escapeHtml(participant)}</span>
                        <button type="button" class="participant-remove" aria-label="Remove ${escapeHtml(participant)}" title="Remove participant">×</button>
                      </li>
                    `
                  )
                  .join("")}
      ...
      activitiesList.querySelectorAll(".participant-remove").forEach((button) => {
        button.addEventListener("click", async () => {
          const participantPill = button.closest(".participant-pill");
